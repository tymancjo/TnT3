...
PC_VBB =      [
                (MBB, 5,'mbb'),
                (VBB, int(200/50),'vbb1'), # ~900mm
                (ACB, 4,'acb'),
                (VBB, int(600/50),'vbb2'), # ~200mm
...
A,B,s, L2, _, air = tntS.SolverAdvance(PC_VBB, # element list
                                        1500, # current description - in [A]
                                        air, # air model to be used
                                        35, # initial temperature [deg C]
                                        3*60*60, # analysis time [s]
...
# second analysis
# changing the elements - switch off the ACB current
for el in PC_VBB:
    if el.iD == 'acb':
        el.current = 0
        el.HTC = 0
...
A1,B1,s1, L2, _, air = tntS.SolverAdvance(PC_VBB, # element list
                                        1500, # current description - in [A]
                                        air, # air model to be used
                                        35, # initial temperature [deg C]
                                        3*60*60, # analysis time [s]
...
t = np.array(A+(np.array(A1)+A[-1]).tolist())
...
# debug4.plot(np.array(air.dT_array))
# debug4.set_title('dT')
...
fig3 = plt.figure('HeatMaps')
cLimits = [b.min(),b.max()]
# Defining the subplot for geometry heat map
hm_t = np.linspace(t[0],t[-2],40)
frames = len(hm_t)
fx = math.ceil(frames / 8.0)
fy = math.ceil(frames / fx)

pict = []
for i,ta in enumerate(hm_t):
    idx = np.argmax(t>ta)
    print(f"t:{ta}, idx:{idx}, Tidx:{t[idx]}")
    
    pict.append(fig3.add_subplot(fx,fy,i+1, aspect='equal'))
    boxes = tntS.drawElements(pict[-1],PC_VBB,np.array(b[idx,:])) # ,cLimits=cLimits)
    pict[-1].set_title(ta)
    pict[-1].axes.get_xaxis().set_visible(False)
    pict[-1].axes.get_yaxis().set_visible(False)

# ta = 1
# idx = np.argmax(t>ta)
# print(f"t:{ta}, idx:{idx}, Tidx:{t[idx]}")
# ax32 = fig3.add_subplot(222, aspect='equal')
# boxes = tntS.drawElements(ax32,PC_VBB,np.array(b[idx,:]),cLimits=cLimits)
# ax32.set_title(ta)
...
# boxes = tntS.drawElements(ax33,PC_VBB,np.array(b[idx,:]),cLimits=cLimits)
# ax33.set_title(ta)
...
# boxes = tntS.drawElements(ax34,PC_VBB,np.array(b[idx,:]),cLimits=cLimits)
# ax34.set_title(ta)
